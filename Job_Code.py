...
job_df_melted = GetData('job.csv').melt(id_vars=[
    'Country Name', 'Country Code', 'Series Name', 'Series Code'], var_name='Year', value_name='Value')
...
job_df_pivoted = job_df_melted.groupby(['Country Name', 'Country Code', 'Year', 'Series Name'])[
    'Value'].mean().unstack().reset_index()

# Creating a pivot table from pivoted_df
job_df_countries = job_df_melted.pivot_table(
    index=['Year', 'Series Name'], columns='Country Name', values='Value')
job_df_years = job_df_melted.pivot_table(
...
job_df_cleaned_new = job_df_cleaned.drop(
    ['Year', 'Country Name'], axis='columns')
...
def Gdp_Line_Plot(Job_Data):
    ...
    # Filter data for the specified countries and years
    filtered_gdp_data = gdp_data[(gdp_data['Country Name'].isin(
        countries)) & (gdp_data['Year'].isin(years))]

    # Pivot the data for line chart
    pivoted_gdp_data = filtered_gdp_data.pivot(
        index='Year', columns='Country Name', values='GDP growth (annual %)')

    # Plot the line chart
    plt.figure(figsize=(10, 6))
    for country in countries:
        plt.plot(pivoted_gdp_data.index,
                 pivoted_gdp_data[country], label=country)
...
def Tax_Line_Plot(Job_Data):
    ...
    # Filter data for the specified countries and years
    filtered_tax_data = tax_data[(tax_data['Country Name'].isin(
        countries)) & (tax_data['Year'].isin(years))]

    # Pivot the data for line chart
    pivoted_tax_data = filtered_tax_data.pivot(
        index='Year', columns='Country Name', values='Tax revenue (% of GDP)')

    # Plot the line chart
    plt.figure(figsize=(10, 6))
    for country in countries:
        plt.plot(pivoted_tax_data.index,
                 pivoted_tax_data[country], label=country)
...
def Germany_HeatMap(Job_Data):
    ...
    heatmap = sns.heatmap(Germany_subset.corr(), annot=True,
                          cmap='magma', fmt='.2f', annot_kws={"size": 10})

    # To truncate names of indicators in x axis
    heatmap.set_xticklabels([label[:20] + '...' if len(label)
                            > 20 else label for label in Germany_subset.columns])

    # To truncate names of indicators in y axis
    heatmap.set_yticklabels([label[:20] + '...' if len(label)
                            > 20 else label for label in Germany_subset.columns])
...
def Egypt_HeatMap(Job_Data):
    ...
    # Plot the heatmap
    plt.figure(figsize=(12, 8))
    heatmap = sns.heatmap(Egypt_subset.corr(), annot=True,
                          cmap='ocean', fmt='.2f', annot_kws={"size": 10})

    # To truncate names of indicators in x axis
    heatmap.set_xticklabels([label[:20] + '...' if len(label)
                            > 20 else label for label in Egypt_subset.columns])
...
def Pakistan_HeatMap(Job_Data):
    ...
    # Plot the heatmap
    plt.figure(figsize=(12, 8))
    heatmap = sns.heatmap(Pakistan_subset.corr(), annot=True,
                          cmap='twilight', fmt='.2f', annot_kws={"size": 10})

    # To truncate names of indicators in x axis
    heatmap.set_xticklabels([label[:20] + '...' if len(label)
                            > 20 else label for label in Pakistan_subset.columns])
